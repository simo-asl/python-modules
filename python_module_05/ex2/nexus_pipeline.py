...
class NexusManager:
    ...
    def add_pipeline(self, pipeline_instance: Optional[ProcessingPipeline]) -> None:
...
class JSONAdapter(ProcessingPipeline):
    ...
    def process(self, input_data: Any) -> Union[str, Any]:
        if isinstance(input_data, dict) and "pipeline" in input_data and "data" in input_data:
...
class CSVAdapter(ProcessingPipeline):
    ...
    def process(self, input_data: Any) -> Union[str, Any]:
        formatted_data = {'pipeline': self.__class__.__name__, 'data': input_data}
...
class TransformStage:
    ...
    def _transform_json_data(self, data_container: Dict) -> None:
        ...
        if isinstance(value, float) and self.JSON_VALUE_NORMAL_RANGE[0] <= value <= self.JSON_VALUE_NORMAL_RANGE[1]:
            range_status = 'Normal'
        else:
    ...
    def _transform_stream_data(self, data_container: Dict) -> None:
        ...
        filtered_readings = [value for value in stream_payload if value > self.STREAM_VALUE_THRESHOLD]
        
        reading_count = len(filtered_readings)
        average_value = sum(filtered_readings) / reading_count if reading_count > 0 else 0
...
class OutputStage:
    ...
    def _output_csv_result(self, data_container: Dict) -> None:
        ...
        print(f"Output: User activity logged: {logged_count} actions processed\n")
    ...
    def _output_stream_result(self, data_container: Dict) -> None:
        ...
        print(f"Output: Stream summary: {reading_count} readings, avg: {avg_value:.1f}\n")
...
if __name__ == '__main__':
    try:
        ...
        system_manager.process_data({'format': 'stream', 'data': stream_test_data})
        ...
        print(f"{execution_duration:.5f}s total processing time\n")
